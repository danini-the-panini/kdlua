...
function Tokenizer:fail(message)
  error(message.." ("..self.line_at_start..":"..self.column_at_start..")")
end
...
function Tokenizer:parse_decimal(s)
  ...
  elseif valid_integer(s) then
    ...
    if table.contains(util.NON_INITIAL_IDENTIFIER_CHARS, utf8.sub(s, 1, 1)) then self:fail("Invalid number: "..s) end
    for i = 2,utf8.len(s) do
      if table.contains(util.NON_IDENTIFIER_CHARS, utf8.sub(s, i, i)) then self:fail("Invalid number: "..s) end
    end
    return { type="IDENT", value=s }
  end
end

function Tokenizer:parse_hexadecimal(s)
  if s:match("^[+-]?%x[0-9a-fA-F_]*$") then
    return { type="INTEGER", value=tonumber(munch_underscores(s), 16)}
  end
  self:fail("Invalid hexadecimal: "..s)
end

function Tokenizer:parse_octal(s)
  if s:match("^[+-]?[0-7][0-7_]*$") then
    return { type="INTEGER", value=tonumber(munch_underscores(s), 8)}
  end
  self:fail("Invalid octal: "..s)
end

function Tokenizer:parse_binary(s)
...
function Tokenizer:unescape(str)
  ...
  while i <= utf8.len(str) do
    ...
    elseif c == "\\" then
      ...
      elseif c2 == "u" then
        c2 = char(i+2)
        if c2 ~= "{" then self:fail("Invalid unicode escape") end
        local hex = ""
        local j = i+3
        local cj = char(j)
        ...
        if #hex > 6 or char(j) ~= "}" then self:fail("Invalid unicode escape: \\u{"..hex.."}") end
        local code = tonumber(hex, 16)
        if not code then self:fail("Invalid unicode escape: "..hex) end
        if code < 0 or code > 0x10FFFF or (code >= 0xD800 and code <= 0xDFFF) then
          self:fail(string.format("Invalid code point \\u{%x}", code))
        end
        i = j
...
function Tokenizer:_read_next()
  ...
  while true do
    ...
    elseif self.context == "rawstring" then
      if c == nil or c == "" then
        self:fail("Unterminated rawstring literal")
      end
    ...
    elseif self.context == "decimal" then
      ...
      elseif table.contains(util.WHITESPACE, c) or table.contains(util.NEWLINES, c) or c == nil then
        return self:parse_decimal(self.buffer)
      else
        self:fail("Unexpected '"..c.."'")
      end
    elseif self.context == "hexadecimal" then
      if c ~= nil and c:match("[0-9a-fA-F_]") then
        ...
        self:traverse(1)
      elseif table.contains(util.WHITESPACE, c) or table.contains(util.NEWLINES, c) or c == nil then
        return self:parse_hexadecimal(self.buffer)
      else
        self:fail("Unexpected '"..c.."'")
      end
    elseif self.context == "octal" then
      if c ~= nil and c:match("[0-7_]") then
        ...
        self:traverse(1)
      elseif table.contains(util.WHITESPACE, c) or table.contains(util.NEWLINES, c) or c == nil then
        return self:parse_octal(self.buffer)
      else
        self:fail("Unexpected '"..c.."'")
      end
    elseif self.context == "binary" then
      if c ~= nil and c:match("[01_]") then
        ...
        self:traverse(1)
      elseif table.contains(util.WHITESPACE, c) or table.contains(util.NEWLINES, c) or c == nil then
        return self:parse_binary(self.buffer)
      else
        self:fail("Unexpected '"..c.."'")
      end
    ...
    elseif self.context == nil then
      ...
      self:fail("Unexpected context "..self.context)
    end
  end
end
...
function Tokenizer:char(i)
  ...
  for _, value in pairs(util.FORBIDDEN) do
    if c == value then self:fail("Forbidden character: "..c) end
  end
  return c
end
...
function Tokenizer:expect_newline(i)
  ...
  elseif not table.contains(util.NEWLINES, c) then
    self:fail("Expected NEWLINE, found '"..c.."'")
  end
  return c
end
